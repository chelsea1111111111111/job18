...
class MainActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.TPFL -> {
                val yellowFragment = supportFragmentManager.beginTransaction()
                yellowFragment.replace(R.id.content, YellowFragment())
                yellowFragment.commit()
                drawer.close()
            }
            R.id.to -> {
                val fragmentTo = supportFragmentManager.beginTransaction()
                fragmentTo.replace(R.id.content, BlueFragment())
                fragmentTo.commit()
                drawer.close()
            }
            R.id.pplg -> {
